...
export default function ProsAndCons() {
  ...
  return (
    <section className="relative w-full bg-gradient-to-b from-gray-900 to-gray-800 dark:from-[#030712] dark:to-gray-900/50 text-white dark:text-gray-100 py-20 px-4 sm:px-6 lg:px-8">
      <div className="container mx-auto max-w-6xl">
        {/* Section Header */}
        <div className="mb-16 text-center">
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold mb-6 text-white dark:text-white">
            {t('prosAndCons.title')}
          </h2>
        ...
        {/* Pros Section */}
        <div className="mb-20">
          <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-green-600 dark:text-green-400 mb-8 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {pros.map((pro) => (
              <div
                key={pro.id}
                className="group p-6 rounded-lg bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 hover:shadow-lg dark:hover:shadow-xl hover:border-green-400 dark:hover:border-green-600 transition-all duration-300"
              >
        ...
        {/* Cons Section */}
        <div>
          <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-red-600 dark:text-red-400 mb-8 flex items-center gap-2">
            <span className="text-2xl">❌</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {cons.map((con) => (
              <div
                key={con.id}
                className="group p-6 rounded-lg bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 hover:shadow-lg dark:hover:shadow-xl hover:border-red-400 dark:hover:border-red-600 transition-all duration-300"
              >
                <div className="text-4xl mb-4 group-hover:scale-110 transition-transform duration-300">
                  {con.icon}
                </div>
                <h4 className="text-lg font-bold text-gray-900 dark:text-white mb-2 group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors duration-300">
                  {con.title}
                </h4>
                <p className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed">
                  {con.description}
