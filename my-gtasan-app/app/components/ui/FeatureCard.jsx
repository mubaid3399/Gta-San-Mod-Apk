'use client';

import { motion } from 'framer-motion';

const variantClasses = {
  default:
    'bg-gray-50 dark:bg-gray-900/30 border-gray-200 dark:border-gray-800 hover:border-[#00ff87]/50',
  blue: 'bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800 hover:border-blue-400',
  amber: 'bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800 hover:border-amber-400',
};

export default function FeatureCard({
  icon,
  title,
  description,
  children,
  variant = 'default',
}) {
  return (
    <motion.div
      whileHover={{ y: -5 }}
      transition={{ duration: 0.3 }}
      className={`p-4 rounded-lg border ${variantClasses[variant]} hover:shadow-md transition-all`}
    >
      {icon && (
        <div className="text-2xl mb-3">
          {typeof icon === 'string' ? <span>{icon}</span> : icon}
        </div>
      )}

      <h4 className="font-bold text-gray-900 dark:text-white mb-2">{title}</h4>

      {description && <p className="text-sm text-gray-700 dark:text-gray-300">{description}</p>}

      {children && <div className="text-sm text-gray-700 dark:text-gray-300">{children}</div>}
    </motion.div>
  );
}
